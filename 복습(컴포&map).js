...
// `shoes.map()`을 사용할 때 `i`는 현재 반복 중인 요소의 인덱스를 나타냅니다. 이 값을 자식 컴포넌트(`Sneakers`)에 **`props`로 전달**해, 내부에서 이미지나 텍스트 등을 해당 인덱스에 맞게 렌더링할 수 있습니다.
...
// 이 코드는 `Sneakers` 컴포넌트로 `i`를 넘기며, `Sneakers`에서 그 값을 이용해 해당 신발의 정보나 이미지를 정확하게 표시합니다.
              })
            }
